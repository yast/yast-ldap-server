...
module Yast
  class LdapDatabaseClass < Module
    def main
      ...
      Yast.include self, "ldap-server/helps.rb"
      Yast.include self, "ldap-server/widgets.rb"
    ...
    def AddDbBasic(createDefaults)
      ...
      while true
        ...
        if ret == :next
          ...
          rc = LdapServer.CheckSuffixAutoCreate(
            Ops.get_string(db, "suffix", "")
          )
    ...
    def DbPpolicyRead(dbindex)
      ...
      if Builtins.size(ppolicy_map) != 0
        ...
        olddb = LdapServer.ReadDatabase(dbindex)
    ...
    def DbPpolicyWrite(dbindex)
      if UI.QueryWidget(:cb_ppolicy_overlay, :Value) == true
        ...
        if pp_default != ""
          db = deep_copy(@baseDb)
          db = LdapServer.ReadDatabase(dbindex) if Ops.greater_than(dbindex, 0)
          ...
          ppolicyEntry = LdapServer.ReadPpolicyDefault(
            Ops.get_string(db, "suffix", "")
          )
          ...
          if Ops.greater_than(Builtins.size(ppolicyEntry), 0)
            ...
            LdapServer.WritePpolicyDefault(
              Ops.get_string(db, "suffix", ""),
    ...
    def DbPpolicyInput(handler_cmd, dbindex)
      if handler_cmd == :cb_ppolicy_overlay
        ...
        db = deep_copy(@baseDb)
        db = LdapServer.ReadDatabase(dbindex) if dbindex != -1
        if Convert.to_boolean(UI.QueryWidget(:cb_pp_append_basedn, :Value))
          pp_default = Ops.add(
            Ops.add(pp_default, ","),
            Ops.get_string(db, "suffix", "")
        ...
        ppolicy = LdapServer.ReadPpolicyDefault(
          Ops.get_string(db, "suffix", "")
        )
        ppolicyEntry = {}
        if Ops.greater_than(Builtins.size(ppolicy), 0)
          ...
          Ldap.LDAPInit
          pw = ""
          authinfo = LdapServer.ReadAuthInfo(Ops.get_string(db, "suffix", ""))
        ...
        if ret == :ok
          ...
          if type == :who_dn || type == :who_dn_subtree || type == :who_group
            ...
            if Ops.get_string(res, "value", "") == ""
              ...
              if !LdapServer.ValidateDn(Ops.get_string(res, "value", ""))
                Popup.Error(
                  Ops.add(
                    Ops.add(
      ...
      while true
        ...
        if ret == :ok
          ...
          if Ops.get_string(what, "what", "") == "base" ||
            ...
            if !LdapServer.ValidateDn(Ops.get_string(what, "dn", ""))
              Popup.Error(
                Ops.add(
    ...
    def DbAclInput(handler_cmd, dbindex)
      db = {}
      db = LdapServer.ReadDatabase(dbindex) if Ops.greater_or_equal(dbindex, 0)
      suffix = Ops.get_string(db, "suffix", "")
    ...
    def DbSyncConsCheck(index)
      ...
      if UI.QueryWidget(:cb_syncrepl, :Value) == true
        ...
        db = deep_copy(@baseDb)
        db = LdapServer.ReadDatabase(index) if Ops.greater_or_equal(index, 0)
        suffix = Ops.get_string(db, "suffix", "")
        testparm = {}
