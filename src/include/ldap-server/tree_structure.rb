...
# $Id$
module Yast
  module LdapServerTreeStructureInclude
    def initialize_ldap_server_tree_structure(include_target)
      Yast.import "UI"
      ...
      Yast.include include_target, "ldap-server/widgets.rb"
    ...
    def cb_read_daemon
      ...
      enabled = LdapServer.ReadServiceEnabled
    ...
    def cb_write_daemon
      ...
      LdapServer.WriteProtocolListenerEnabled(
        "ldap",
    ...
    def cb_read_schema
      ...
      inclist = LdapServer.ReadSchemaList
    ...
    def cb_input_schema
      ...
      if @handler_cmd == :pb_add
        ...
        if Builtins.regexpmatch(new_item, ".*.schema$")
          ...
          inclist2 = LdapServer.ReadSchemaList
          update_schemalist(inclist2)
        elsif !LdapServer.AddLdifToSchemaList(new_item)
          err = LdapServer.ReadError
          @callback_error = Ops.add(
            Ops.add(Ops.get(err, "msg", ""), "\n"),
            Ops.get(err, "details", "")
        ...
        if LdapServer.IsSchemaDeletable(current_item)
    ...
    def cb_write_loglevel
      ...
      LdapServer.WriteLogLevels(idlist)
    end
    ...
    def cb_read_allow
      ...
      allowlist = LdapServer.ReadAllowFeatures
      disallowlist = LdapServer.ReadDisallowFeatures
    ...
    def cb_read_tls
      ...
      UI.ChangeWidget(
        ...
        :Value,
        LdapServer.ReadProtocolListenerEnabled("ldaps")
      )
      UI.ChangeWidget(
        :te_ca_file,
        :Value,
    ...
    def cb_input_tls
      ...
      common_cert_available = LdapServer.HaveCommonServerCertificate
    ...
    def cb_read_databases
      ...
      dblist = LdapServer.ReadDatabaseList
    ...
    def cb_write_db
      ...
      if Ops.get_string(db, "rootdn", "") != "" &&
        ...
        olddb = LdapServer.ReadDatabase(index)
      ...
      res = LdapServer.UpdateDatabase(index, db)
    ...
    def cb_input_db
      ...
      if @handler_cmd == :pb_changepw
        ...
        if newpw != nil
          ...
          newhash = LdapServer.HashPassword(
    ...
    def cb_write_confdb
      ...
      if Ops.get_boolean(db, "secure_only", false)
        if Ops.get_string(olddb, "rootpw", "") == ""
          ...
          if pw != ""
            Builtins.y2milestone("set password")
            newhash = LdapServer.HashPassword(hashAlgo, pw)
            Builtins.y2milestone("new hash: %1", newhash)
    ...
    def cb_read_bdb_index
      ...
      idxMap = LdapServer.ReadDatabaseIndexes(index)
      i = 0
      newItems = []
    ...
    def cb_read_acl
      ...
      treeItem = @current_tree_item
      index = Ops.get_integer(@widget_map, [@current_tree_item, "index"])
      acllist = LdapServer.ReadDatabaseAcl(index)
    ...
    def cb_write_acl
      ...
      ret = LdapServer.ChangeDatabaseAcl(
        index,
        Convert.convert(
          changedAcls,
          :from => "list <map>",
    ...
    def cb_read_syncprov
      ...
      treeItem = @current_tree_item
      index = Ops.get_integer(@widget_map, [@current_tree_item, "index"])
      sp = LdapServer.ReadSyncProv(index)
    ...
    def cb_read_synccons
      ...
      treeItem = @current_tree_item
      index = Ops.get_integer(@widget_map, [@current_tree_item, "index"])
      srl = LdapServer.ReadSyncRepl(index)
      if Ops.greater_than(Builtins.size(srl), 1)
        ...
        sr = Ops.get(srl, 0, {})
        updateref = LdapServer.ReadUpdateRef(index)
    ...
    def cb_write_synccons
      ...
      result = false
      result = LdapServer.WriteSyncRepl(
        index,
        Ops.get_map(syncrepl, "syncrepl", {})
      )
      if !result
      ...
      LdapServer.WriteUpdateRef(index, Ops.get_map(syncrepl, "updateref", {}))
      result
    end
    ...
    def cb_write_ppolicy
      ...
      if Ops.greater_than(Builtins.size(ppolicy), 0)
        ...
        ppolicy_old = LdapServer.ReadPpolicyOverlay(index)
