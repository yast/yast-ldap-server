...
# $Id$
module Yast
  module LdapServerMirrormodeWizardInclude
    def initialize_ldap_server_mirrormode_wizard(include_target)
      Yast.import "UI"
      ...
      Yast.include include_target, "ldap-server/wizards.rb"
    ...
    def MirrorModeOverview
      ...
      serverids = LdapServer.ReadServerIds
      pos = -1
      ...
      while true
        ...
        if ret == :back || ret == :abort || ret == :cancel || ret == :next ||
          ...
          if selected != nil
            ...
            if Ops.get_string(url, "host", "") == LdapServer.ReadHostnameFQ
              Popup.Error(
                _(
